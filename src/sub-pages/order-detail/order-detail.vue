...
<template>
	<view class="h-full pb-[50px]">
		<app-scroll-view class="h-[calc(100%-50px)]" :padding-bottom="50" :refresher-enabled="false"
			...
			<view class="bg-[#fbb957] h-40 mb-2">
				...
				<template v-if="judgeState(OrderState.PendingPayment)">
					...
					<view class="center text-white text-sm">
						...
						<uni-countdown splitor-color="#fff" color="#fff" :showColon="false" :show-day="false"
							:second="orderDetail?.countdown" start @timeup="onTimeUp"></uni-countdown>
					</view>
				...
				<template v-else-if="judgeState(OrderState.PendingDelivery)">
					...
					<view class="around mt-3 px-7">
						<view class="bg-white py-1.5 px-10 text-[#fbb957] rounded-full" @click="handleBuyAgain">再次购买</view>
						<view class="bg-white py-1.5 px-10 text-[#fbb957] rounded-full" @click="handleConfirmDelivery">确认收货</view>
					</view>
				</template>
				...
				<view class="bg-white p-3 rounded-1 mx-2">
					<view class="center-y py-2" v-for="(address, index) in logisticsList" :key="address.id">
						<view class="w-10 center">
							...
							<image v-else-if="index == 0 && orderDetail?.orderState == OrderState.Completed" class="w-7 h-7"
			...
			sku.attrsText
				...
				<view class="flex justify-end items-center">
					<view v-if="orderDetail?.orderState == OrderState.PendingReview"
						class="text-xs text-[#666] border-1 border-solid border-[#ddd] w-fit py-1 px-5.5 mr-2 rounded-full">
						去评价
					</view>
					<view v-if="orderDetail?.orderState || 0 > OrderState.PendingShipment"
			...
			<view class="bg-white mx-2 px-2 py-3 rounded-1">
				...
				<view class="center-y text-[#666] py-2 text-sm">
					...
					<view class="ml-1 text-[#666] w-fit py-0.25 px-2 border-1 border-solid border-[#e2e2e2] text-xs rounded-full"
						@click="copyOrderID">复制</view>
				</view>
			...
			<view class="fixed z-1 left-0 text-sm right-0 bottom-0 h-50px bg-white shadow-2xl center-y justify-end px-3">
				<template v-if="judgeState(OrderState.PendingPayment)">
					<view class="w-fit py-2 px-6 rounded-full border-1 border-solid border-[#ddd] mr-4 text-[#666]"
						@click="cancelOrder">
						取消订单
					</view>
				...
				<template v-else-if="judgeState(OrderState.Cancelled)">
					<view class="w-fit py-2 px-6 rounded-full border-1 border-solid border-[#ddd] mr-4 text-red"
						@click="deleteOrder">
						删除订单
					</view>
					<view class="text-[#fbb957] border-1 border-solid border-[#fbb957] w-fit py-2 px-6 rounded-full"
	...
	<app-popup ref="appPopupRef" title="取消订单原因">
		...
		<view class="center mt-3">
			<view class="w-fit py-2 px-13 rounded-full border-1 border-solid border-[#ddd] mr-4 text-[#666]"
				@click="cancelReason">取消</view>
