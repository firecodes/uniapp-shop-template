...
<template>
	<view class="mx-3 bg-white mt-3 mb-6">
		...
		<app-form-item label="性别">
			<uni-data-checkbox v-model="componentProfile.gender" selectedColor="#fbb957"
				:localdata="sexRange"></uni-data-checkbox>
		</app-form-item>
		<app-form-item label="生日">
			<picker placeholder="请选择生日时间" :value="componentProfile.birthday" mode="date" start="1900-01-01" :end="endDate"
		...
		<app-form-item label="城市">
			...
			<uni-data-picker placeholder="请选择地址" popup-title="请选择城市" collection="opendb-city-china"
				field="code as value, name as text" orderby="value asc" :step-searh="true" :border="false" self-field="code"
				parent-field="parent_code" @change="handleCityPickerChange">
				<view v-if="componentProfile.fullLocation">{{ componentProfile.fullLocation }}</view>
				<view v-else class="text-[#808080] text-sm">请选择地址</view>
			</uni-data-picker>
			<!-- #endif -->
