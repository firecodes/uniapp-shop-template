...
<template>
	<view class="relative">
		<view class="flex bg-white">
			<view class="flex-1 py-3 center" @click="handleTitleClick(orderState.type)" v-for="orderState in orderStateList"
				:key="orderState.type">
				{{ orderState.title }}
			</view>
		</view>
		<view class="absolute left-0 bottom-0 w-20% h-0.75 px-10 bg-[#fbb957] transition-all duration-400"
			:style="{ left: Number(currentType) * 20 + '%' }"></view>
	</view>
