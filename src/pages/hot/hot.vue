...
<template>
	...
	<view v-else class="hot">
		...
		<template v-for="(subType, index) in subTypes" :key="subType.id">
			<app-scroll-view class="flex-1 h-full overflow-hidden" v-show="currentIndex == index" :refresher-enabled="false"
				:scrolltolower="handleScrollToLower">
				<view class="flex-wrap around">
					<app-shop-item-v1 v-for="shop in subType?.goodsItems?.items" :guess-like="shop"
